...
| 22.10.04(화) | 타임라인 기능 구상 및 UI 구성                                | 스크롤 뷰로 안만들어도 테이블뷰로 만들 수 있잖아?!           |
| 22.10.05(수) | 타임라인 UI에 데이터 뿌려주기                                | Image가 제대로 먹지 않는다...                                |
| 22.10.06(목) | 발표 준비 및 타임라인 오류 해결                              | 내일까지 Apple 심사 완료가 되길....                          |
...
업데이트로 미뤄놨던 타임라인 기능을 이제는 만들어야 한다고 생각했고, 어떻게 구현할까 고민했다. 타임라인에 보여줄 정보로는 코인을 획득한 시간, 어떻게 코인을 얻게 되었는지, 몇 코인을 획득했거나 소비했는지, 그 날의 집중 시간에 따른 이미지를 보여주자고 생각했고, UI를 구성했다.
...
각 부분에 필요한 데이터를 넣어주고, 분기처리를 통해 타임라인에 보여줄 수 있도록 만들어 주었다. 처음엔 문제없이 실행되는 줄 알았으나 image 보여지는 부분에 원하는 이미지가 출력이 되지 않는 것을 확인했고, filter을 새로 repo에서 만들어주고 적용시켜주었다.
...
- 최종 발표 전 심사 제출 및 발표준비

#### 최종 발표 전 심사 제출 및 발표준비

추후 업데이트는 지속적으로 할 예정이지만, 최종 발표 전 마지막 업데이트 제출 및 발표 준비를 하였다. 발표를 여러명이서 하다보니 너무 기능위주의 발표보다는 조금 유쾌한(?) 발표로 준비했다. 이렇게 첫 프로젝트의 마지막 개발 일지를 작성하다보니, 하루도 빠짐없이 다 작성했다는 뿌듯함과, 다음에는 조금 더 기획도 철저히하고, 공수 산정 및 코드의 구조화도 잘 해보고 싶다는 욕심이 생겼다. 개발일지는 추후에 업데이트 할 때에 다시 작성하겠지만, 첫 개발 시작일인 9월 12일부터 10월 6일까지 하루도 쉬지 않고 개발을 하고 일지를 작성했다는 경험은 앞으로의 개발 활동에 있어서 큰 경험으로 다가올 것 같다. 이렇게 SeSAC에서 함께 한 첫 출시 프로젝트는 우당탕탕 성공(?)출시로 마무리 된 것 같아 재밌고, 소중한 경험으로 기억될 것 같다.
